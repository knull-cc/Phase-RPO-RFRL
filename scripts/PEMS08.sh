#!/usr/bin/env bash
# PEMS08 (170 nodes). Usage: bash scripts/PEMS08.sh [--model X ...]
...
for pred_len in 12 24 48 96; do
  python -u run.py \
    ...
    --data PEMS --root_path ./dataset/PEMS/ --data_path PEMS08.npz --freq h \
    --model_id PEMS08_${seq_len}_${pred_len} --model "$model_name" \
