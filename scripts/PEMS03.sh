#!/usr/bin/env bash
# PEMS03 (358 nodes). Usage: bash scripts/PEMS03.sh [--model X ...]
...
for pred_len in 12 24 48 96; do
  python -u run.py \
    ...
    --data PEMS --root_path ./dataset/PEMS/ --data_path PEMS03.npz --freq h \
    --model_id PEMS03_${seq_len}_${pred_len} --model "$model_name" \
